...
def random_padding(raw_image, random_ratio=(0.0, 0.05)):
    left = int(raw_image.shape[1] * random.uniform(random_ratio[0], random_ratio[1]))
    right = int(raw_image.shape[1] * random.uniform(random_ratio[0], random_ratio[1]))
    top = int(raw_image.shape[0] * random.uniform(random_ratio[0], random_ratio[1]))
    bottom = int(raw_image.shape[0] * random.uniform(random_ratio[0], random_ratio[1]))
    np_image = tf.pad(raw_image, tf.stack([[top, bottom], [left, right], [0, 0]])).numpy()
    return np_image
